...
func (pr *PortResponse) setTenantID(vmi *models.VirtualMachineInterface, vn *models.VirtualNetwork) {
	if vmi.GetParentType() != models.KindProject {
		pr.TenantID = vn.GetParentUUID()
	} else {
		pr.TenantID = vmi.GetParentUUID()
	}
	pr.TenantID = strings.Replace(pr.TenantID, "-", "", -1)
}
