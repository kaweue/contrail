	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.readData.vnReq != nil {
				mockServ.EXPECT().GetVirtualNetwork(gomock.Any(), gomock.Any()).Return(tt.readData.vnReq, nil)
			}
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: &PortResponse{
				...
				Name:                "default-vmi",
				AllowedAddressPairs: []string(nil),
				AdminStateUp:        false,
...
func TestPortUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			readRes, err := tt.port.Update(nil, rp)
